...
(() => {
	...
	const syncTheme = (iframe) => {
		const theme = currentTheme();
		const url = new URL(iframe.src || iframe.dataset.tutSrc, location.href);
	...
	const wireFrame = (iframe) => {
		if (iframe.dataset.simplicityEmbedWired) {
			syncTheme(iframe);
			return;
		}
	...
	new MutationObserver(wireAll).observe(document.body, { attributes: true, attributeFilter: ['data-md-color-scheme'] });
