...
//! explorer, and save it as fixture files in the layout
//! `crates/simplicity-runner/tests/fixtures/` and `export_tx_fixtures` expect.
//!
//! Two modes:
//!
//!     cargo run --manifest-path crates/tx-fixtures/Cargo.toml --bin add_tx_fixture -- <txid>
//!
//! fetches one specific transaction — the normal way to proactively cache a new
//! example before committing it. With no argument:
//!
//!     cargo run --manifest-path crates/tx-fixtures/Cargo.toml --bin add_tx_fixture
//!
...
//! get interpreted in this project.
//!
//! Lives in its own crate, deliberately: `simplicity-runner`'s `[lib]` target is what
//! `wasm-pack build` compiles for the browser, and `wasm-pack` builds every target in
//! that package for `wasm32-unknown-unknown`, bin targets included. This tool's
//! dependencies (`ureq`, real network sockets) have no business being dragged into that
//! build, let alone actually compiling for it.
//!
...
//!
//!     cargo run --manifest-path crates/tx-fixtures/Cargo.toml --bin export_tx_fixtures
...
fn main() {
    ...
    // A sibling crate, not this one's own `tests/` — the fixtures belong to
    // simplicity-runner (its `cargo test` reads them directly), this crate just fetches
    // them.
    let fixtures_dir = manifest_dir.join("../simplicity-runner/tests/fixtures");
    ...
    match env::args().nth(1) {
        Some(txid) => {
            ...
            println!();
            println!("Next steps:");
            println!("  cargo run --manifest-path crates/tx-fixtures/Cargo.toml --bin export_tx_fixtures");
            println!("  cargo test --manifest-path crates/simplicity-runner/Cargo.toml");
        }
        None => {
            // Two levels up from `crates/tx-fixtures/` is the repo root.
