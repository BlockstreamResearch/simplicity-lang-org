...
//! per `tx-*.hex` fixture in `crates/simplicity-runner/tests/fixtures/`, in exactly the
//! shape `js/explorer.ts`'s `LoadedTransaction` expects, so the client only needs a
//! plain `fetch` + `JSON.parse` for a preset txid — no client-side transaction parsing
//! at all.
//!
...
//! dependencies (`ureq`, real filesystem access) have no business being dragged into
//! that build, let alone actually compiling for it.
...
fn main() {
    ...
    // simplicity-runner (its `cargo test` reads them directly), this crate just exports
    // them.
    ...
    let out_dir = manifest_dir.join("../..").join("docs/assets/tx-fixtures");

    // Every `prev-*.hex` fixture, indexed by its own txid, exactly as `build_tx_env`
    // does in `simplicity-runner/src/lib.rs` — inputs are matched by txid, not filename.
